...
#	connected = False
	...
	buff_size = 2**16
	dns_socket_recv = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	dns_socket_recv.bind((find_local_ip(), DNS_PORT_RECV))
	dns_socket_recv.setblocking(0)
	data = None
	addr = None
	print('new clients: ' + str(dns_socket_recv.getsockname()))
	while True:
#		if not connected:
...
#			fp = open('up', 'a')
#			fp.write('waiting for new clients\n')
#			fp.close()		
#			print('send something')
			data, addr = dns_socket_recv.recvfrom(buff_size)
#			print('hi')
			print(addr)
			print(data)
...
#				connected = False
		if addr == (DNS_HOST, DNS_PORT_RECV):
			new_clients += data
		if (new_clients != '') and (not pipe.full()):
			fp = open('up', 'a')
			fp.write(str(clients) + '\n')
			fp.close()
...
#	dns_socket_send = None
	dns_socket_send = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
	dns_socket_send.bind((find_local_ip(), DNS_PORT_SEND))
	while True:
		if not connected:
...
#			dns_socket_send.connect((DNS_HOST, DNS_PORT_SEND))
		...
		try:
			dns_socket_send.sendto(results, (DNS_HOST, DNS_PORT_SEND))
