...
#	s.listen(len(replicas))
	print('broadcasting from: ' + str(CLIENTS_BROADCAST_PORT))
	global clients, clients_lock

	while True:
#		if len(connections) != len(replicas):
			
#		while len(connections) != len(replicas):
#			conn, addr = s.accept()
...
#					connections.append(conn)
	...
	#	clients_lock.acquire()
	#	try:
	##		new_clients = list(clients - clients_encountered)
	#		new_clients = clients.copy()
	#	finally:
	#		clients_lock.release()
		try:	
...
#					conn.send(' '.join(new_clients))
#				print(clients)
				for host in replicas:
					s.sendto(' '.join(new_clients), (host, CLIENTS_BROADCAST_PORT))
				fp = open('ips', 'w+')
				fp.write(' '.join(new_clients) + '\n')
				fp.close()
		except socket.error:
			pass
		...
		with mappings_lock:
			...
			for key in new_mappings.keys():
				try:
					...
					mappings[key] = new_mappings[key]
			fp = open('up', 'a')
			fp.write('after update:\n' + str(mappings) + '\n')
			fp.close()
		fp = open('up', 'a')
		fp.write('updated using:\n--\n' + str(new_mappings) + '\n--\n')
		fp.close()
...
def main():
	...
	if len(sys.argv) != number_of_args + 1:
		...
		if port > 40000:
			...
			#< 40k
			pass
		print('1:' + str(MAPPINGS_UPDATE_PORT))
		print('2:' + str(CLIENTS_BROADCAST_PORT))
